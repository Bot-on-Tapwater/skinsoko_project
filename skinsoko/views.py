...
class HelperFunctions:
    ...
    def paginate_results(request, query_results, view_url, items_per_page=40):
        ...
        if page_obj.has_previous():
            if "page=" in view_url:
                json_data["previous_page"] = (
                    f"{view_url[:view_url.rfind('page=')]}page={page_obj.previous_page_number()}"
                )
            else:
                json_data["previous_page"] = (
                    f"{view_url}?page={page_obj.previous_page_number()}"
                )

        if page_obj.has_next():
            if "page=" in view_url:
                json_data["next_page"] = (
                    f"{view_url[:view_url.rfind('page=')]}page={page_obj.next_page_number()}"
                )
            else:
                json_data["next_page"] = (
                    f"{view_url}?page={page_obj.next_page_number()}"
                )
...
class AddressViews:
    ...
    def get_user_saved_addresses(request):
        ...
        if user_saved_addresses.exists():
            return JsonResponse(
                HelperFunctions.paginate_results(
                    request, [address for address in user_saved_addresses], view_url
                ),
                safe=False,
            )
        else:
    ...
    def add_address_to_user_profile(request):
        try:
            ...
            return JsonResponse(
                {"error": f"User with ID: {userId} does not exist."}, status=404
            )
        ...
        try:
            ...
            for field, value in data.items():
                ...
                if hasattr(address_to_update, field):
                    ...
                    return JsonResponse(
                        f"There is no field named {field} in address table.", safe=False
                    )
            ...
            return JsonResponse(
                {"error": f"Address with ID: {id} does not exist."}, status=404
            )
...
class CartViews:
    ...
    def get_contents_of_shopping_cart_of_user(request):
        ...
        try:
            if id:
                ...
                cart_contents_of_user = ShoppingCart.objects.get(
            ...
            cart_summary["orderTotal"] += (
                cart_summary["itemsSubtotal"]
                + cart_summary["estimatedTax"]
                + cart_summary["shippingFee"]
            )

            return JsonResponse(
                {"cart_summary": cart_summary, "cart_items": cart_items}
            )
    ...
    def add_product_to_user_cart(request, productId):
        try:
            ...
            if not id:  # user not authenticated
                ...
                try:
                    session_cart, created = ShoppingCart.objects.get_or_create(
                        session_key=session_key
                    )
                    try:
                        cart_item = CartItem.objects.get(
                            cart=session_cart, product=product
                        )
                        cart_item.quantity += int(quantity)
                    except CartItem.DoesNotExist:
                        cart_item = CartItem(
                            cart=session_cart, product=product, quantity=quantity
                        )
                    cart_item.save()
                    return JsonResponse({"success": True}, safe=False)
                except Exception as e:
            ...
            try:
                try:
                    existing_cart_item = CartItem.objects.get(
                        cart=ShoppingCart.objects.get(user=id), product=product
                    )
            ...
            return JsonResponse(
                {"error": f"Product with ID: {productId} not found."}, status=404
            )
    ...
    def update_product_in_user_cart(request, productId):
        try:
            ...
            if int(quantity) > product.quantity_in_stock:
                return JsonResponse(
                    f"Quantity in stock is {product.quantity_in_stock}. Please reduce the quantity.",
                    safe=False,
                )

            if id:
                cart_item, created = CartItem.objects.get_or_create(
                    cart__user_id=id, product_id=productId
                )
            else:
                cart_item, created = CartItem.objects.get_or_create(
                    cart__session_key=session_key, product_id=productId
                )
    ...
    def update_product_quantity(userId):
        try:
            ...
            return JsonResponse(
                {"error": f"User with ID: {userId} does not have an active cart."},
                status=404,
            )
    ...
    def remove_product_from_user_cart(request, productId):
        try:
            ...
            return JsonResponse(
                {"error": f"User with ID: {id} does not have an active cart."},
                status=404,
            )

        except Product.DoesNotExist:
            return JsonResponse(
                {"error": f"Product with ID: {productId} not found in cart."},
                status=404,
            )
    ...
    def clear_entire_shopping_cart(request):
        try:
            ...
            return JsonResponse(
                {"error": f"User with ID: {id} does not have a cart."}, status=404
            )
...
class AuthViews:
    ...
    def register_view(request):
        if request.method == "POST":
            try:
                ...
                login_response = AuthViews.login_view(request, email, password)

                AuthViews.send_registration_email(email)
        ...
        if request.method == "POST" or (email and password):
            try:
                ...
                if check_password(password, user.password):
                    ...
                    CartViews.merge_carts(request, user)
    ...
    def request_password_reset(request):
        if request.method == "POST":
            try:
                ...
                AuthViews.send_password_reset_email(user, reset_link)
    ...
    def validate_passsword_reset_token(request):
        try:
            ...
            AuthViews.reset_password(request, password, user.email)
...
class CategoryViews:
    ...
    def get_list_of_all_main_categories(request):
        ...
        if list_of_all_main_categories.exists():
            return JsonResponse(
                [category.to_dict() for category in list_of_all_main_categories],
                safe=False,
            )
        else:
            return JsonResponse(None, safe=False)
    ...
    def get_list_of_all_sub_categories_in_a_main_category(request, main_category):
        ...
        try:
            list_of_subcategories = SubCategory.objects.filter(
                main_category=MainCategory.objects.get(name=main_category)
            ).order_by("name")
            return JsonResponse(
                [category.to_dict() for category in list_of_subcategories], safe=False
            )
    ...
    def get_list_of_all_brands(request):
        ...
        if list_of_all_brands.exists():
            return JsonResponse(
                [brand.to_dict() for brand in list_of_all_brands], safe=False
            )
        else:
            return JsonResponse(None, safe=False)
...
class CouponViews:
    ...
    def create_coupons(num_coupons, discount):
        ...
        for _ in range(num_coupons):
            code = CouponViews.generate_random_code()
    ...
    def send_coupon_email(email):
        ...
        coupon = CouponViews.get_coupon()  # Adjust as per your Coupon model
        ...
        # Send email
        HelperFunctions.send_email(subject, message, recipient_list=[email])

    @staticmethod
    def generate_and_save_coupons():
        # Generate 100 codes with 10% discount
        CouponViews.create_coupons(1000, 10.00)

    @staticmethod
    def generate_coupons(request):
        CouponViews.generate_and_save_coupons()
        ...
        if request.method == "POST":
            try:
                ...
                if coupon_code is None:
                    return JsonResponse(
                        {"error": "No coupon code provided."}, status=400
                    )
                ...
                try:
                    ...
                    if coupon.active:
                        return JsonResponse(
                            {"discount": float(coupon.discount)}, status=200
                        )
                    else:
                        return JsonResponse(
                            {"error": "Coupon is not active."}, status=400
                        )
                except Coupon.DoesNotExist:
...
class DataViews:
    ...
    def consolidated_data_view(request):
        consolidated_data = DataViews.extract_json_data(
            DataViews.consolidated_data_no_sesssion_or_user_data(request)
        )
        user_status_response = AuthViews.user_status(request)
        cart_response = CartViews.get_contents_of_shopping_cart_of_user(request)

        consolidated_data["user_status"] = DataViews.extract_json_data(
            user_status_response
        )
        consolidated_data["cart"] = DataViews.extract_json_data(cart_response)
    ...
    def consolidated_data_no_sesssion_or_user_data(request):
        brands_response = CategoryViews.get_list_of_all_brands(request)
        categories_response = CategoryViews.get_list_of_all_main_categories(request)
        brands_data = DataViews.extract_json_data(brands_response)
        categories_data = DataViews.extract_json_data(categories_response)

        subcategories = [
            {
                category.name: DataViews.extract_json_data(
                    CategoryViews.get_list_of_all_sub_categories_in_a_main_category(
        ...
        return JsonResponse(
            {
                ...
                "best_selling_products": [
                    product.to_dict()
                    for product in Product.objects.filter(best_seller=True)
                ],
                "brands": brands_data,
...
class DatabaseViews:
    ...
    def populate_categories(request):
        try:
            ...
            if not required_columns.issubset(df.columns):
                return JsonResponse(
                    {"error": "CSV file must contain BRANDS and CATEGORIES columns"},
                    status=400,
                )
                ...
                for col in df.columns:
                    if col not in ["BRANDS", "CATEGORIES"] and pd.notna(row[col]):
                        ...
                        SubCategory.objects.get_or_create(
                            name=subcategory_name, main_category=main_category
                        )

            return JsonResponse(
                {"message": "Database populated successfully"}, status=200
            )
    ...
    def populate_towns(request):
        try:
            ...
            if "Town Name" not in df.columns or "Delivery Fee" not in df.columns:
                return JsonResponse(
                    {
                        "error": "CSV file must contain Town Name and Delivery Fee columns"
                    },
                    status=400,
                )
            ...
            for index, row in df.iterrows():
                ...
                print(
                    f"Processing row: Town Name: {town_name}, Delivery Fee: {delivery_fee}"
                )

                # Create or get the town
                town, created = Towns.objects.get_or_create(
                    name=town_name, defaults={"delivery_fee": delivery_fee}
                )

                if created:
                    print(
                        f"Created new town: {town.name} with delivery fee: {town.delivery_fee}"
                    )
                else:
    ...
    def read_csv_and_create_dict(csv_file_path):
        ...
        for index, row in df.iterrows():
            try:
                print(
                    f"Processing row {index}: {row.to_dict()}"
                )  # Print row data for debugging

                subcategories = [
                    subcategory.strip() for subcategory in row["Categories"].split(",")
                ]
                ...
                product = {
                    ...
                    "Selling Price": (
                        int(row["Selling Price"].replace(",", ""))
                        if isinstance(row["Selling Price"], str)
                        else row["Selling Price"]
                    ),
    ...
    def populate_database(request):
        try:
            DatabaseViews.populate_towns(request)
            DatabaseViews.populate_categories(request)
            DatabaseViews.populate_products(request)
            CouponViews.generate_coupons(request)
    ...
    def maillist_create(request):
        ...
        if request.method == "POST":
            ...
            try:
                ...
                CouponViews.send_coupon_email(email)
                return JsonResponse(
                    {"message": "Maillist entry created successfully"}, status=201
                )
            except IntegrityError:
                # Handle case where email already exists
                return JsonResponse(
                    {"error": "Email already exists in Maillist"}, status=400
                )
            except Exception as e:
    ...
    def maillist_all(request):
        if request.method == "GET":
            try:
                ...
                data = [
                    {"email": entry.email, "phone_number": entry.phone_number}
                    for entry in maillist_entries
                ]
...
class OrderViews:
    ...
    def list_orders_placed_by_user_with_user_id(request):
        ...
        try:
            ...
            return JsonResponse(
                [order.to_dict() for order in orders_placed_by_user], safe=False
            )
    ...
    def get_list_of_all_orders(request):
        ...
        if all_orders.exists():
            return JsonResponse(
                HelperFunctions.paginate_results(
                    request, [order for order in all_orders], view_url
                ),
                safe=False,
            )
        else:
            return JsonResponse(
                {"error": f"No orders found, add an order and try again."}, status=404
            )
    ...
    def get_details_of_order_with_order_id(request, id):
        try:
            ...
            return JsonResponse(
                {"error": f"Order with ID: {id} does not exist."}, status=404
            )
    ...
    def get_order_items_for_order_with_order_id(request, id):
        ...
        return JsonResponse(
            [order_item.to_dict() for order_item in specific_order_items], safe=False
        )
    ...
    def create_new_order(request):
        try:
            ...
            if not cartitems:
                return JsonResponse(
                    {"error": f"User has no items in cart."}, status=404
                )

            else:
                total_cost_of_cart_items = CartItem.objects.filter(
                    cart=user_cart
                ).aggregate(
                    total_cost=Sum(
                        ExpressionWrapper(
                            F("quantity") * F("product__discounted_price"),
                ...
                new_order_items = list(
                    map(
                        ...
                        [item for item in CartItem.objects.filter(cart=user_cart)],
                    )
                )
...
class PesapalViews:
    ...
    def get_pesapal_token_view(request):
        return JsonResponse(PesapalViews.get_pesapal_token(), safe=False)
    ...
    def register_ipn():
        ...
        headers = {
            ...
            "Authorization": "Bearer " + PesapalViews.get_pesapal_token()["token"],
        }
        ...
        if response.status_code == 200:
            ...
            return JsonResponse(
                {"error": "Failed to register IPN."}, status=response.status_code
            )
    ...
    def pesapal_submit_order(request, order_id):
        ...
        if response.status_code == 200:
            ...
            return JsonResponse(
                {"error": "Failed to submit order."}, status=response.status_code
            )
    ...
    def ipn_notification_view(request):
        try:
            ...
            if not order_tracking_id:
                ...
                return JsonResponse(
                    {"error": "Order tracking ID not found."}, status=400
                )

            response = PesapalViews.pesapal_transaction_status(
                request, order_tracking_id
            )
            if response.status_code == 200:
                response_data = response.json()
                print(response_data)
                if response_data["status_code"] == 1:
                    ...
                    CartViews.update_product_quantity(userId)
                    print("clear shopping cart")
                    CartViews.clear_entire_shopping_cart_helper_function(userId)
                    ...
                    return JsonResponse(
                        {"error": "Failed to process payment."}, status=500
                    )
            else:
                logger.error(
                    f"Failed to query payment status. Response: {response.text}"
                )
                return JsonResponse(
                    {"error": "Failed to query payment status."}, status=500
                )
    ...
    def register_ipn_view(request):
        return JsonResponse(PesapalViews.register_ipn(), safe=False)
...
class ProductViews:
    ...
    def list_all_products(request):
        ...
        if filter_main_category:
            all_products = all_products.filter(
                subcategories__main_category__name=filter_main_category
            )
        ...
        if all_products.exists():
            return JsonResponse(
                HelperFunctions.paginate_results(request, all_products, view_url),
                safe=False,
            )
        else:
    ...
    def get_product_with_product_id(request, slug):
        try:
            ...
            return JsonResponse(
                {"error": f"Product with ID: {id} was not found."}, status=404
            )
    ...
    def search(request):
        ...
        return JsonResponse(
            HelperFunctions.paginate_results(request, search_results, view_url),
            safe=False,
        )
...
class ReviewViews:
    ...
    def get_reviews_for_product_with_product_id(request, slug):
        ...
        if reviews.exists():
            ...
            return JsonResponse(
                {"current_page": 0, "total_pages": 0, "query_results": []}
            )
    ...
    def list_reviews_created_by_user_with_user_id(request):
        ...
        try:
            ...
            reviews_by_user = Review.objects.filter(
                user=user_id
            )  # Assuming you have a 'user' field in your Review model

            if reviews_by_user.exists():
                return JsonResponse(
                    [review.to_dict() for review in reviews_by_user], safe=False
                )
            else:
                return JsonResponse(None, safe=False)
    ...
    def create_review_for_product_with_product_id(request, slug):
        try:
            ...
            existing_review = Review.objects.filter(
                product=product_to_review, user=user_leaving_review
            ).first()
            ...
            if existing_review:
                ...
                return JsonResponse(
                    {
                        "error": "You can only submit one review per product.Check your profile for more actions."
                    },
                    status=400,
                )
            ...
            new_review = Review(
                product=product_to_review,
                user=user_leaving_review,
                rating=rating,
                comment=comment,
                full_name=full_name,
            )
            ...
            return JsonResponse(
                {"error": f"Product with ID: {id} does not exist."}, status=404
            )
    ...
    def user_delete_review(request, id):
        try:
            ...
            return JsonResponse(
                {"error": f"Review with ID: {id} does not exist."}, status=404
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=500)
...
class SelcomViews:
    ...
    def generate_selcom_headers(data):
        ...
        string_to_sign = f"timestamp={timestamp}&" + "&".join(
            [f"{key}={value}" for key, value in data.items()]
        )

        # Generate the digest
        digest = hmac.new(
            settings.SELCOM_API_SECRET.encode(), string_to_sign.encode(), hashlib.sha256
        ).digest()
        digest_base64 = base64.b64encode(digest).decode()

        # Generate the authorization header
        authorization = (
            f"SELCOM {base64.b64encode(settings.SELCOM_API_KEY.encode()).decode()}"
        )
    ...
    def create_minimal_order(request):
        ...
        try:
            print(
                f"BASEURL: {settings.SELCOM_BASE_URL}, API_KEY: {settings.SELCOM_API_KEY}, API_SECRET: {settings.SELCOM_API_SECRET}"
            )

            client = apigwClient.Client(
                settings.SELCOM_BASE_URL,
                settings.SELCOM_API_KEY,
                settings.SELCOM_API_SECRET,
            )
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Invalid response from Selcom API"}, status=500
            )
        ...
        try:
            ...
            try:
                if payment_status == "COMPLETED":
                    ...
                    return JsonResponse(
                        {"message": "Payment status updated and email sent"}, status=200
                    )
                else:
                    logger.warning(f"Order {order_id} payment status: {payment_status}")
                    return JsonResponse(
                        {"message": "Payment status not completed"}, status=200
                    )
            except Order.DoesNotExist:
    ...
    def get_selcom_order_status(request):
        try:
            ...
            if not order_id:
                return JsonResponse(
                    {"error": "order_id parameter is required"}, status=400
                )
        ...
        try:
            ...
            for field, value in QueryDict(request.body).items():
                if hasattr(user_to_update, field) and field == "password":
                    ...
                    return JsonResponse(
                        {"error": f"There is no field named {field} in users table."},
                        status=404,
                    )
            ...
            return JsonResponse(
                {"error": f"user with ID: {id} was not found."}, status=404
            )
...
class WishlistViews:
    ...
    def add_item_to_wishlist(request, productId):
        ...
        try:
            ...
            return JsonResponse(
                {"message": "Item added to wishlist successfully"}, status=200
            )
            ...
            return JsonResponse(
                {"error": f"Product with ID: {productId} does not exist."}, status=404
            )
